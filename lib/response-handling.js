...
function processResponse(model, sp, samlResponse) {
	...
	let doc = new DOMParser().parseFromString(samlResponse.payload, "text/xml");

	// choose the first Issuer node from the document, which
	// should reflect the assertion's IDP
	const issuer = select("//saml:Issuer/text() | //saml2:Issuer/text()", doc)[0];
	if (!issuer) {
	...
	return model
		...
		.then(resolvedIDP => {
			...
			if (select("//saml:EncryptedAssertion | //saml2:EncryptedAssertion", doc).length) {

				// remove the top-level signature, as it is now invalid
				select("/samlp:Response/ds:Signature | /saml2p:Response/ds:Signature", doc).forEach(sigNode => {
				...
				return encryption
					...
					.then(newDoc => {
						...
						doc = newDoc;
						assertion = select("//saml:Assertion | //saml2:Assertion", doc)[0];
		...
		.then(() => {
			...
			const nameIDNode = select("//saml:Subject/saml:NameID | //saml2:Subject/saml:NameID", assertion)[0];
...
function checkStatus(doc, idp) {
	...
	const statusNode = select("//samlp:StatusCode | //saml2p:StatusCode", doc)[0];
	...
	if (statusCodeMatch[1] == "Success") {
		...
		const messageNodes = select("//samlp:StatusMessage/text() | //saml2p:StatusMessage/text()", doc);
...
function extractUserAttributes(assertion) {
	...
	const attributeStatement = select("//saml:AttributeStatement | //saml2:AttributeStatement", assertion)[0];

	if (attributeStatement) {
		const attributes = select("saml:Attribute | saml2:Attribute", attributeStatement);

		return attributes.map(attrNode => {
			let attrName = attrNode.getAttribute("Name");
			let friendlyName = attrNode.getAttribute("FriendlyName");
			let attrVals = select("saml:AttributeValue/text() | saml2:AttributeValue/text()", attrNode)
				.map(n => n.nodeValue)
				.filter(n => n);
