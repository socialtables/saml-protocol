...
function processAuthnRequest(model, idp, samlRequest) {
	...
	let doc = new DOMParser().parseFromString(samlRequest.payload, "text/xml");
	...
	return model
		...
		.then(resolvedSP => {
			...
			const requestObj = {
				idp: idp,
				sp: sp,
				requestID: select("//samlp:AuthnRequest | //saml2p:AuthnRequest", doc)[0].getAttribute("ID")
			};

			// attach nameID policy if specified
			const nameIDPolicyNode = select("//samlp:NameIDPolicy | //saml2p:NameIDPolicy", doc)[0];
			if (nameIDPolicyNode) {
				requestObj.nameID = {
