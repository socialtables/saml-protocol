...
ResponseValidator.prototype.validateResponseDocument = function(doc) {
	...
	const responseNodes = select("//samlp:Response | //saml2p:Response", doc);
	...
	const issuer = select("saml:Issuer/text() | saml2:Issuer/text()", responseNode).toString();
	...
	return Promise.resolve()
		...
		.then(() => {
			const assertion = select("saml:Assertion | saml2:Assertion", responseNode)[0];
			if (!assertion) {
...
ResponseValidator.prototype.validateAssertion = function(assertion) {
	...
	const issuer = select("saml:Issuer/text() | saml2:Issuer/text()", assertion).toString();
...
ResponseValidator.prototype.validateSubjectConfirmation = function(assertion) {
	...
	const subjectConfirmation = select("//saml:SubjectConfirmation | //saml2:SubjectConfirmation", assertion)[0];
	...
	const data = select("//saml:SubjectConfirmationData | //saml2:SubjectConfirmationData", subjectConfirmation)[0];
	if (!data) {
...
ResponseValidator.prototype.validateConditions = function(assertion) {
	...
	const conditions = select("//saml:Conditions | //saml2:Conditions", assertion)[0];
	...
	const audienceRestriction = select("saml:AudienceRestriction | saml2:AudienceRestriction", conditions)[0];
	if (audienceRestriction) {
		const resolvedEntityID = urlParse(this.sp.entityID).href;
		const audiences = select("saml:Audience | saml2:Audience", audienceRestriction);
...
ResponseValidator.prototype.validateAuthnStatement = function(assertion) {
	...
	const authnStatements = select("//saml:AuthnStatement | //saml2:AuthnStatement", assertion);
