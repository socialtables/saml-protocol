...
function getDataFromRedirectBinding(searchParams) {
  const samlParamName = searchParams.get('SAMLRequest') ? 'SAMLRequest' : 'SAMLResponse';
  const samlPayload = searchParams.get(samlParamName);
  if (!samlPayload) {
  ...
  if (signature) {
    const signedPayload = constructSignaturePayload(searchParams);
 ...
 */
function constructSignaturePayload(searchParams) {
  const samlParamName = searchParams.get('SAMLRequest') ? 'SAMLRequest' : 'SAMLResponse';
  ...
  [samlParamName, 'RelayState', 'SigAlg']
    ...
    .forEach((p) => {
      payload.set(p, searchParams.get(p));
    });
