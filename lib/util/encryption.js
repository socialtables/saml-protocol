...
function decryptAssertion(doc, credentials) {
	...
	const encryptedAssertion = select("//saml:EncryptedAssertion | //saml2:EncryptedAssertion", doc)[0];
	...
	return decryptData(encryptedDataStr, credentials)
		.then(decryptedXml => {
			...
			const newAssertionDoc = new DOMParser().parseFromString(decryptedXml, "text/xml");
			...
			const newDoc = new DOMParser().parseFromString(newDocXML, "text/xml");
			return newDoc;
		});
}
...
function encryptAssertion(doc, credential, algorithms) {
	...
	const assertion = select("//saml:Assertion | //saml2:Assertion", doc)[0];
	...
	return encryptData(assertXml, credential, algorithms)
		.then(encryptedData => {
			...
			const encryptedAssertion = new DOMParser().parseFromString(encAssertString, "text/xml");
			// replace the assertion with the encrypted node
			const enclosingNode = assertion.parentNode;
			enclosingNode.replaceChild(encryptedAssertion.documentElement, assertion);
...
function encryptData(data, credential, algorithms) {
	return new Promise(function (resolve, reject) {
		...
		const encryptOptions = {
			...
			keyEncryptionAlgorithm: algs.keyEncryption || defaultAlgorithms.keyEncryption,
			pem: certificate,
			rsa_pub: publicKey
		};
