...
function createEncryptionKeyDescriptorJSON(certificate, algorithms) {
	...
	if (!selectedAlgorithms) {
		selectedAlgorithms = []
			.concat(encryption.supportedAlgorithms.encryption)
			.concat(encryption.supportedAlgorithms.keyEncryption);
	}
	return {
		"md:KeyDescriptor": [
			{ "@use": "encryption" },
...
function getCredentialsFromRoleDescriptor(roleDescriptor) {
	...
	select("md:KeyDescriptor", roleDescriptor).forEach(keyDescriptor => {
		...
		const formattedCert = pemFormatting.addPEMHeaders("certificate", x509Cert.nodeValue);
